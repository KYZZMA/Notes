...
public class MainActivity extends AppCompatActivity implements PopupMenu.OnMenuItemClickListener {
    ...
    private void filter(String newText) {
        List<Notes> filteradList = new ArrayList<>();
        for (Notes singleNotes : notes){
            ...
            || singleNotes.getNotes().toLowerCase().contains(newText.toLowerCase())){
                filteradList.add(singleNotes);
            }

        }
        notesListAdapter.filterList(filteradList);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 101){
            if (resultCode == Activity.RESULT_OK){
                ...
                database.mainDAO().isert(new_notes);
                notes.clear();
    ...
    private void updateRecycle(List<Notes> notes) {
        ...
        notesListAdapter = new NotesListAdapter(MainActivity.this,notes,notesClickListen);
        recyclerView.setAdapter(notesListAdapter);

    }
    private final NotesClick notesClickListen = new NotesClick() {
        @Override
